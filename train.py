...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(model_save_path, "sb3_ppo"), exist_ok=True)

    for run in range(10):
        env = StockEnv(dataset, **env_configs)
        monitor_file = os.path.join(run_save_path, f"sb3_ppo_{run}.csv")

        env = Monitor(env, monitor_file)
        model = BPPO('MlpPolicy', env)
        model.learn(total_timesteps=150000, progress_bar=True)
        env.close()
        model.save(os.path.join(model_save_path, "sb3_ppo", f"{run}.zip"))
